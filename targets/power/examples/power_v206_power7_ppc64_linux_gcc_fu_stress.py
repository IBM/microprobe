...
def main_setup():
    ...
    for instr in target.isa.instructions.values():
        ...
        for unit in instr.execution_units:
            ...
            if unit not in func_units:
                func_units[unit] = [
                    elem
                    for elem in target.elements.values()
                    if elem.name == unit
                ][0]
    ...
    cmdline.add_option(
        ...
        "L",
        2,
        "Average latency of the selected instructins. " "Default: 2 cycles",
...
def _generate_benchmark(target, output_prefix, args):
    ...
    try:
        ...
        fname = f"{output_prefix}{'_'.join(func_unit_names)}"
        fname = f"{fname}_LAT_{latency}"
        fname = f"{fname}_DEP_{distance}"
