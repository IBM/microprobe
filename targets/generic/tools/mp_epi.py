...
def _generic_policy_wrapper(all_arguments):
    ...
    bname = f"{bname}#DI_{kwargs['data_init']}"

    if GLOBAL_POLICY is not None:
        policy = GLOBAL_POLICY
    elif MICROPROBE_RC["debugwrapper"]:
    ...
    elif target.name.endswith("linux_gcc_nolibc"):

        wrapper_name = "CWrapperNoLibC"
        wrapper_class = _get_wrapper(wrapper_name)
        wrapper = wrapper_class(reset=kwargs["reset"])
        extension = "c"
    ...
    if kwargs["skip"] and os.path.isfile(outputfile):
        print_info(f"{outputfile} already generated!")
        return False

    if kwargs["skip"] and os.path.isfile(f"{outputfile}.gz"):
        print_info(f"{outputfile} already generated! (compressed).gz")
        return False
    ...
    for prop in [
        ...
        if (
            hasattr(instruction, prop)
            and getattr(instruction, prop)
            and not kwargs["all"]
        ):
            print_info(f"{instruction.name} not skip (reason: {prop})!")
            return False
...
def _main(arguments):
    ...
    if (
        "epi_output_file" not in arguments
