...
# Classes
class dma2objdump(TestCase):  # pylint: disable=invalid-name
    """dma2objdump test class."""

    _multiprocess_can_split_ = True

    name = "mp_dma2objdump"
    description = "mp_dma2objdump tool tests"
    cmd = [os.path.join(BASEPATH,
                        "targets", "generic", "tools", "mp_dma2objdump.py")]
    ...
    def setUp(self):
        tempfile = mkstemp(prefix="microprobe_%s_" % self.name, suffix=".dump")
        os.close(tempfile[0])
    ...
    def test_001(self):
        """
        dma2objdump_test001 on riscv_v22-riscv_generic-riscv64_linux_gcc
        """
        self._wrapper("riscv_v22-riscv_generic-riscv64_linux_gcc",
                      self.filenames[0],
                      extra="-h")
    ...
    def _wrapper(self, target, filename, extra=None):
        ...
        for trial in range(0, self.trials):
            ...
            error_code = subprocess.call(
                test_cmd,
                stdout=open(self.filenames[0], "w"),
                stderr=tfile
            )
            if error_code == 0:
                break
