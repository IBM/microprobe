...
def process_autosummary_toc(app, doctree):
    ...
    def crawl_toc(node, depth=1):
        crawled[node] = True
        for dummy_j, subnode in enumerate(node):
            try:
...
class Autosummary(Directive):
    ...
    def run(self):
        ...
        if 'toctree' in self.options:
            ...
            docnames = []
            for dummy_name, dummy_sig, dummy_summary, real_name in items:
...
def limited_join(sep, items, max_chars=30, overflow_marker="..."):
    ...
    *overflow_marker*.
    ...
    n_chars = 0
    n_items = 0
    for dummy_j, item in enumerate(items):
...
def autolink_role(dummy_typ, rawtext, etext, lineno, inliner,
                  options={}, content=[]):
    """Smart linking role.
    ...
    prefixes = get_import_prefixes_from_env(env)
    try:
        dummy_name, dummy_obj, dummy_parent, dummy_modname = import_by_name(
            pnode['reftarget'], prefixes)
    except ImportError:
        content = pnode[0]
...
def process_generate_options(app):
    ...
    from myautosummary.generate \
        import generate_autosummary_docs  # @UnresolvedImport
