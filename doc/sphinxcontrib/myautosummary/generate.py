...
def main(argv=sys.argv):
    ...
    generate_autosummary_docs(None, args, options.output_dir,
                              "." + options.suffix,
...
def generate_autosummary_docs(app, sources, output_dir=None, suffix='.rst',
    ...
    for name, path, template_name in sorted(set(items), key=str):
        ...
        with open(fn, 'w') as f:
            ...
            def get_members2(obj, include_public=[]):
...
def find_autosummary_in_docstring(name, module=None, filename=None):
    ...
    """
    try:
        dummy_real_name, obj, dummy_parent, dummy_modname = import_by_name(
            name)
        ...
        print("Failed to import '%s': %s (%s)" % (name, e, module))
